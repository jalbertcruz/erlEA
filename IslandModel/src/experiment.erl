...
config(PopulationSize, ChromosomeSize, NumberOfIterations) ->
  {A1, A2, A3} = now(),
  random:seed(A1, A2, A3),
  #imodelGA{
      population = genInitPop(PopulationSize, ChromosomeSize), %% Population of the each island
      ...
      selectNewPopulation = fun model:selectNewPopulation/2, %% function to obtain the new individuals for the nex iteration
...
run(GraphicalNode) ->
  C32 = config(512, 32, 20),
  C64 = config(512, 64, 20),
  C128 = config(512, 128, 20),
  Notif = fun(Id, Pop) ->
    manager ! {iterate, Id, length([Ind || {Ind, Eval} <- Pop, cond80({Ind, Eval})])}
  end,
