...
loop({InitEvol, EndEvol}, Iterations, Emigrations) ->

  receive

    {migration, {Ind, Fit}, T} ->
      loop({InitEvol, EndEvol}, Iterations, [{Ind, Fit, T} | Emigrations]);
    ...
    {iteration, Population} ->
      ...
      loop({InitEvol, EndEvol}, [{Min, Max, Ave} | Iterations], Emigrations);

    {initEvol, T} ->
      loop({T, EndEvol}, Iterations, Emigrations);

    {endEvol, T} ->
      loop({InitEvol, T}, Iterations, Emigrations);
